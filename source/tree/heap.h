...
public:
    ...
    void add(int element) {
        int index = arrayList->getSize();
        int parentIndex = getParent(index);
        int parent = arrayList->getElementAt(parentIndex + 1);

        arrayList->addAt(element, index + 1);

        while (element < parent) {
            arrayList->swap(index, parentIndex);
            index = parentIndex;
            parentIndex = getParent(index);
            parent = arrayList->getElementAt(parentIndex + 1);
        }
    }
    ...
    int removeMin(){
        ...
        // swap the last with the minimum
        ...
        while (true) {
            ...
            if (leftIndex < size && leftChild <
                    arrayList->getElementAt(smallerIndex + 1)) {
                smallerIndex = leftIndex;
            }

            if (rightIndex < size && rightChild <
                    arrayList->getElementAt(smallerIndex + 1)) {
